...
def index(request):
	context_dict = {
		...
		'others' : Participant.objects.filter(trainees__in=Training.objects.all()).distinct().filter(designation__contains='Others'),
...
def chart5(request, instn_id):
	cursor = connection.cursor()
	cursor.execute('SELECT DISTINCT T1.participant_id AS participant_id,(T1.score*0.4+T2.score*0.6) AS final,T1.sname AS sname,T1.fname AS fname,T1.training_id AS training_id,T1.topic AS topic FROM (SELECT AVG(score) AS score,training_id,participant_id,sname,fname,GROUP_CONCAT(topic) AS topic FROM plotter_activity,plotter_participant,plotter_training,plotter_module WHERE plotter_activity.participant_id=plotter_participant.ID AND training_id=plotter_training.ID AND plotter_training.module_id=plotter_module.ID AND category="Quiz" AND instn_id=%s AND designation LIKE "BS%" GROUP BY participant_id) AS T1 LEFT JOIN (SELECT AVG(score) AS score,training_id,participant_id,sname,fname,GROUP_CONCAT(topic) AS topic FROM plotter_activity,plotter_participant,plotter_training,plotter_module WHERE plotter_activity.participant_id=plotter_participant.ID AND training_id=plotter_training.ID AND plotter_training.module_id=plotter_module.ID AND category="Lab" AND instn_id=%s AND designation LIKE "BS%" GROUP BY participant_id) AS T2 ON T1.training_id=T2.training_id AND T1.participant_id=T2.participant_id ORDER BY final DESC LIMIT 12', [instn_id, instn_id])
	results = []
	col_names = [name[0] for name in cursor.description]
	for row in cursor.fetchall():
		rowset = []
...
def training_related(request, training_id):
	...
	if request.user.username == 'dost':
		...
		cursor.execute('SELECT T1.ppant_id AS ppant_id, T1.sname AS sname, T1.fname AS fname, T1.designation AS designation, T1.email AS email, T1.contactn AS contactn, T1.instn_id AS instn_id, T1.abbrev AS abbrev, T2.score AS quiz, T3.score AS lab FROM (SELECT plotter_participant.*,plotter_training_trainee.participant_id AS ppant_id,plotter_participant.instn_id AS instn_id,plotter_institution.abbrev FROM plotter_participant,plotter_institution,plotter_training,plotter_training_trainee WHERE plotter_participant.instn_id=plotter_institution.ID AND plotter_training_trainee.participant_id=plotter_participant.ID AND plotter_training_trainee.training_id=plotter_training.ID AND plotter_training.ID=%s) AS T1 LEFT JOIN (SELECT * FROM plotter_activity where plotter_activity.training_id=%s AND category="Quiz") AS T2 ON T1.ppant_id=T2.participant_id LEFT JOIN (SELECT * FROM plotter_activity where plotter_activity.training_id=%s AND category="Lab") AS T3 ON T1.ppant_id=T3.participant_id ORDER BY abbrev,designation,sname,fname', [training_id, training_id, training_id])
